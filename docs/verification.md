...
Status: all five implementation slices completed locally with synthetic data. No real OpenRouter request, spending, remote publishing, or old implementation import. This is a local release candidate, not an assertion that every v1 compatibility requirement is finished.
...
| Credential custody | TLS mock receives fixture auth; responses, owner readback, generated config, identity files, audit and container logs are checked for leakage. DB/WAL payloads and private keys are encrypted. Reflected upstream secrets remain an exclusion. |
...
The real OpenCode → connector → Budge → OpenRouter generation smoke test needs separately authorized credentials, a selected model and a spend allowance. The installed OpenCode user wrapper loads a real key; tests bypassed that wrapper and did not inspect its credential store. OpenCode 1.18.20 loaded the generated provider configuration and its resolved configuration contained the connector base URL and `budge-local`. This is configuration validation, not a live-provider generation test.

Publishing, repository creation/remote setup and release licensing remain owner decisions outside this implementation authorization. There is no Git remote configured and no release has been published.
