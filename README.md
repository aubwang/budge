...
Certificates last 30 days. The connector checks renewal at startup and hourly, renewing in the final seven days while preserving the device ID. It atomically replaces the encrypted identity file, then switches new TLS handshakes to the new certificate. The server renews its leaf under the same trust root before expiry. Expired devices need a fresh invitation. Replacing the trust root requires re-enrollment.

Finite MCP dispatch has a two-minute total deadline. Revocation prevents claims made afterward; an already claimed/running request may still reach the provider. On restart, interrupted dispatches become `outcome_unknown` and are never resent automatically. To recover a restored backup, start with `server --recover-restored` before allowing any traffic. This revokes all prior devices, deletes owner sessions and cancels pending/queued requests. Re-enroll and review access from scratch, and reconcile provider effects manually. Lost deduplication history cannot be reconstructed.
